...
ui<-fluidPage( titlePanel("Welcome!!"),
              ...
              br(),
              br(),
              plotOutput(outputId = 'nameplot',height = '600px',width='40%')
...
server<-function(input,output){
  ...
  output$nameplot<-renderPlot(
    babynames%>%
      ...
      ggplot(aes(x=year,y=n))+geom_line()+scale_x_continuous(limits=input$year) +theme_minimal()+ylab('Frequency of names')+ggtitle('Nameplot accross time period')+ theme_bw() +
